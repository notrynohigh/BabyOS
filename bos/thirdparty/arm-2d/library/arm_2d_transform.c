...
#define __API_PIXEL_AVERAGE_RESULT_GRAY8() (tPixel >> 8)

#define __API_PIXEL_AVERAGE_RESULT_RGB565() \
    (tPixel.R >>= 8, tPixel.G >>= 8, tPixel.B >>= 8, __arm_2d_rgb565_pack(&tPixel))

#define __API_PIXEL_AVERAGE_RESULT_CCCN888()                                               \
    (tPixel.R >>= 8, tPixel.G >>= 8, tPixel.B >>= 8, tPixel.A = *((uint8_t *)pTarget + 3), \
 ...
 * Utilities                                                                  *
 *----------------------------------------------------------------------------*/
__arm_2d_point_adj_alpha_t __arm_2d_point_get_adjacent_alpha_fp(arm_2d_point_float_t *ptPoint)
{
    B_ASSERT(NULL != ptPoint);
                ...
                [1] =
                    {
                        ...
                        .chAlpha = (uint8_t)(((float)iXSign + (float)x)          //!< x
                                             * ((float)(1 - iYSign) - (float)y)  //!< y
                                             * 256.0f),
                    },
                [2] =
                    {
                        ...
                        .chAlpha = (uint8_t)(((float)(1 - iXSign) - (float)x)  //!< x
...
#if 1
    tResult.tMatrix[0].chAlpha =
        256 - tResult.tMatrix[1].chAlpha - tResult.tMatrix[2].chAlpha - tResult.tMatrix[3].chAlpha;
#endif
    return tResult;
}

__arm_2d_point_adj_alpha_t __arm_2d_point_get_adjacent_alpha_q16(arm_2d_point_fx_t *ptPoint)
{
    B_ASSERT(NULL != ptPoint);
                        ...
                        [1] =
                            {
                                ...
                                .chAlpha =
                                    (uint8_t)__USAT(MUL_Q16(MUL_Q16(
                                                                (TO_Q16(iXSign) + x)       //!< x
                                                                ,
                                                                (TO_Q16(1 - iYSign) - y))  //!< y
                                                            ,
                        ...
                        [2] =
                            {
                                ...
                                .chAlpha =
                                    (uint8_t)__USAT(MUL_Q16(MUL_Q16((TO_Q16(1 - iXSign) - x)  //!< x
                                                                    ,
                                                                    (TO_Q16(iYSign) + y))     //!< y
                                                            ,
                                                            TO_Q16(256)) >>
                        ...
                        [3] =
                            {
                                ...
                                .chAlpha =
                                    (uint8_t)__USAT(MUL_Q16(MUL_Q16((TO_Q16(iXSign) + x)   //!< x
                                                                    ,
                                                                    (TO_Q16(iYSign) + y))  //!< y
                                                            ,
                                                            TO_Q16(256)) >>
...
static void __arm_2d_transform_regression(arm_2d_size_t *__RESTRICT ptCopySize,
                                          arm_2d_location_t *pSrcPoint, float fAngle, float fScale,
                                          arm_2d_location_t *tOffset, arm_2d_location_t *center,
...
#define ONE_BY_2PI_Q31 341782637.0f
    ...
    tPointCornerFx[0][0].Y =
        __QDADD(__QDADD(centerQ16.Y, MUL_Q16(iYQ16, cosAngleFx)), MUL_Q16(iXQ16, sinAngleFx));
    tPointCornerFx[0][0].X =
        __QDSUB(__QDADD(centerQ16.X, MUL_Q16(iXQ16, cosAngleFx)), MUL_Q16(iYQ16, sinAngleFx));
...
static void __arm_2d_transform_regression(arm_2d_size_t *__RESTRICT ptCopySize,
                                          arm_2d_location_t *pSrcPoint, float fAngle, float fScale,
                                          arm_2d_location_t *tOffset, arm_2d_location_t *ptCenter,
                                          arm_2d_rot_linear_regr_t regrCoefs[])
{
    int_fast16_t iHeight = ptCopySize->iHeight;
    int_fast16_t iWidth  = ptCopySize->iWidth;
...
ARM_NONNULL(1, 2, 5)
static arm_2d_point_float_t *__arm_2d_transform_point(const arm_2d_location_t *ptLocation,
                                                         ...
                                                         __arm_2d_transform_info_t *ptTransform)
{
    ...
    do
    {
        ...
        __arm_2d_transform_point(&tCornerPoint, &ptTransform->tCenter, ptTransform->fAngle,
                                 ptTransform->fScale, &tPoint);
...
static void __arm_2d_transform_preprocess_target(arm_2d_op_trans_t       *ptThis,
                                                 const arm_2d_location_t *ptTargetCentre)
{
    __arm_2d_transform_info_t *ptTransform =
        (__arm_2d_transform_info_t *)((uintptr_t)ptThis +
...
#else
    //! equivalent code
    B_ASSERT(NULL == this.Target.ptRegion);
...
arm_fsm_rt_t __arm_2d_gray8_sw_transform_with_colour_keying(__arm_2d_sub_task_t *ptTask)
{
    ...
    B_ASSERT(ARM_2D_COLOUR_8BIT == OP_CORE.ptOp->Info.Colour.chScheme);

    ARM_TYPE_CONVERT(ptTask->Param.tCopyOrig.tOrigin.pBuffer, intptr_t) -=
        ptTask->Param.tCopyOrig.tOrigin.nOffset;

    __arm_2d_impl_gray8_transform(&(ptTask->Param.tCopyOrig), &this.tTransform);
...
arm_fsm_rt_t __arm_2d_rgb565_sw_transform_with_colour_keying(__arm_2d_sub_task_t *ptTask)
{
    ...
    __arm_2d_impl_rgb565_transform(&(ptTask->Param.tCopyOrig), &this.tTransform);
...
arm_fsm_rt_t __arm_2d_cccn888_sw_transform_with_colour_keying(__arm_2d_sub_task_t *ptTask)
{
    ...
    __arm_2d_impl_cccn888_transform(&(ptTask->Param.tCopyOrig), &this.tTransform);
...
arm_fsm_rt_t __arm_2d_gray8_sw_transform_only(__arm_2d_sub_task_t *ptTask)
{
    ...
    __arm_2d_impl_gray8_transform_only(&(ptTask->Param.tCopyOrig), &this.tTransform);
...
arm_fsm_rt_t __arm_2d_rgb565_sw_transform_only(__arm_2d_sub_task_t *ptTask)
{
    ...
    __arm_2d_impl_rgb565_transform_only(&(ptTask->Param.tCopyOrig), &this.tTransform);
...
arm_fsm_rt_t __arm_2d_cccn888_sw_transform_only(__arm_2d_sub_task_t *ptTask)
{
    ...
    __arm_2d_impl_cccn888_transform_only(&(ptTask->Param.tCopyOrig), &this.tTransform);
...
arm_2d_err_t arm_2dp_gray8_tile_transform_with_colour_keying_and_opacity_prepare(
    ...
    float fAngle, float fScale, uint_fast8_t chFillColour, uint_fast8_t chOpacity)
{
    B_ASSERT(NULL != ptSource);
    ...
    return __arm_2d_transform_preprocess_source((arm_2d_op_trans_t *)ptThis, &this.tTransform);
}

ARM_NONNULL(2)
arm_2d_err_t arm_2dp_rgb565_tile_transform_with_colour_keying_and_opacity_prepare(
    arm_2d_op_trans_opa_t *ptOP, const arm_2d_tile_t *ptSource, const arm_2d_location_t tCentre,
    float fAngle, float fScale, uint_fast16_t hwFillColour, uint_fast8_t chOpacity)
{
    B_ASSERT(NULL != ptSource);
...
arm_2d_err_t arm_2dp_cccn888_tile_transform_with_colour_keying_and_opacity_prepare(
    arm_2d_op_trans_opa_t *ptOP, const arm_2d_tile_t *ptSource, const arm_2d_location_t tCentre,
    float fAngle, float fScale, uint32_t wFillColour, uint_fast8_t chOpacity)
{
    B_ASSERT(NULL != ptSource);
...
arm_fsm_rt_t __arm_2d_gray8_sw_transform_with_colour_keying_and_opacity(__arm_2d_sub_task_t *ptTask)
{
    ...
    {
        __arm_2d_impl_gray8_transform_with_opacity(&(ptTask->Param.tCopyOrig), &this.tTransform,
                                                   this.chOpacity);
    }
...
arm_fsm_rt_t __arm_2d_rgb565_sw_transform_with_colour_keying_and_opacity(
    __arm_2d_sub_task_t *ptTask)
{
    ...
    {
        __arm_2d_impl_rgb565_transform_with_opacity(&(ptTask->Param.tCopyOrig), &this.tTransform,
                                                    this.chOpacity);
    }
...
arm_fsm_rt_t __arm_2d_cccn888_sw_transform_with_colour_keying_and_opacity(
    __arm_2d_sub_task_t *ptTask)
{
    ARM_2D_IMPL(arm_2d_op_trans_opa_t, ptTask->ptOP);
    B_ASSERT(ARM_2D_COLOUR_SZ_32BIT == OP_CORE.ptOp->Info.Colour.u3ColourSZ);
    ...
    {
        __arm_2d_impl_cccn888_transform_with_opacity(&(ptTask->Param.tCopyOrig), &this.tTransform,
                                                     this.chOpacity);
    }
    return arm_fsm_rt_cpl;
}
...
arm_2d_err_t arm_2dp_rgb565_tile_transform_only_with_opacity_prepare(
    arm_2d_op_trans_opa_t *ptOP, const arm_2d_tile_t *ptSource, const arm_2d_location_t tCentre,
    float fAngle, float fScale, uint_fast8_t chOpacity)
{
    B_ASSERT(NULL != ptSource);
...
arm_fsm_rt_t __arm_2d_gray8_sw_transform_only_with_opacity(__arm_2d_sub_task_t *ptTask)
{
    ...
    {
        __arm_2d_impl_gray8_transform_only_opacity(&(ptTask->Param.tCopyOrig), &this.tTransform,
                                                   this.chOpacity);
    }
...
arm_fsm_rt_t __arm_2d_rgb565_sw_transform_only_with_opacity(__arm_2d_sub_task_t *ptTask)
{
    ...
    {
        __arm_2d_impl_rgb565_transform_only_opacity(&(ptTask->Param.tCopyOrig), &this.tTransform,
                                                    this.chOpacity);
    }
...
arm_fsm_rt_t __arm_2d_cccn888_sw_transform_only_with_opacity(__arm_2d_sub_task_t *ptTask)
{
    ...
    {
        __arm_2d_impl_cccn888_transform_only_opacity(&(ptTask->Param.tCopyOrig), &this.tTransform,
                                                     this.chOpacity);
    }
    return arm_fsm_rt_cpl;
}
...
ARM_NONNULL(2)
arm_fsm_rt_t arm_2dp_tile_transform(arm_2d_op_trans_t *ptOP, const arm_2d_tile_t *ptTarget,
                                    ...
                                    const arm_2d_location_t *ptTargetCentre)
{
    ...
    if (NULL != ptRegion)
    {
        ...
        this.Target.ptTile =
            arm_2d_tile_generate_child(ptTarget, ptRegion, &ptTransform->Target.tTile, false);
                                                                ...
                                                                float fAngle, float fScale)
{
    ...
    arm_2d_err_t tErr = __arm_mask_validate(ptSource, ptSourceMask, NULL, NULL, 0);
    if (tErr < 0)
    {
        return tErr;
    }
...
arm_fsm_rt_t __arm_2d_gray8_sw_transform_with_src_mask(__arm_2d_sub_task_t *ptTask)
{
    ARM_2D_IMPL(arm_2d_op_trans_msk_t, ptTask->ptOP);
    B_ASSERT(ARM_2D_COLOUR_8BIT == OP_CORE.ptOp->Info.Colour.chScheme);

    ARM_TYPE_CONVERT(ptTask->Param.tCopyOrigMask.use_as____arm_2d_param_copy_orig_t.tOrigin.pBuffer,
                     intptr_t) -=
    ...
    bool bIsMaskChannel8In32 =
        (ARM_2D_CHANNEL_8in32 == ptTask->Param.tCopyOrigMask.tOrigMask.tColour.chScheme);

    if (bIsMaskChannel8In32)
    {
        __arm_2d_impl_gray8_transform_with_src_chn_mask(&(ptTask->Param.tCopyOrigMask),
                                                        &this.tTransform);
    }
    else
#endif
    {
        __arm_2d_impl_gray8_transform_with_src_mask(&(ptTask->Param.tCopyOrigMask),
                                                    &this.tTransform);
    }
...
arm_fsm_rt_t __arm_2d_rgb565_sw_transform_with_src_mask(__arm_2d_sub_task_t *ptTask)
{
    ARM_2D_IMPL(arm_2d_op_trans_msk_t, ptTask->ptOP);
    B_ASSERT(ARM_2D_COLOUR_RGB565 == OP_CORE.ptOp->Info.Colour.chScheme);
    ...
    if (bIsMaskChannel8In32)
    {
        __arm_2d_impl_rgb565_transform_with_src_chn_mask(&(ptTask->Param.tCopyOrigMask),
                                                         &this.tTransform);
    }
    else
#endif
    {
        __arm_2d_impl_rgb565_transform_with_src_mask(&(ptTask->Param.tCopyOrigMask),
                                                     &this.tTransform);
    }
...
arm_fsm_rt_t __arm_2d_cccn888_sw_transform_with_src_mask(__arm_2d_sub_task_t *ptTask)
{
    ARM_2D_IMPL(arm_2d_op_trans_msk_t, ptTask->ptOP);
    B_ASSERT(ARM_2D_COLOUR_SZ_32BIT == OP_CORE.ptOp->Info.Colour.u3ColourSZ);
    ...
    if (bIsMaskChannel8In32)
    {
        __arm_2d_impl_cccn888_transform_with_src_chn_mask(&(ptTask->Param.tCopyOrigMask),
                                                          &this.tTransform);
    }
    else
#endif
    {
        __arm_2d_impl_cccn888_transform_with_src_mask(&(ptTask->Param.tCopyOrigMask),
                                                      &this.tTransform);
    }
...
arm_2d_err_t arm_2dp_gray8_tile_transform_with_src_mask_and_opacity_prepare(
    ...
    const arm_2d_tile_t *ptSourceMask, const arm_2d_location_t tCentre, float fAngle, float fScale,
    uint_fast8_t chOpacity)
{
    B_ASSERT(NULL != ptSource);
...
arm_fsm_rt_t __arm_2d_gray8_sw_transform_with_src_mask_and_opacity(__arm_2d_sub_task_t *ptTask)
{
    ...
    else
    {
        ...
        if (bIsMaskChannel8In32)
        {
            __arm_2d_impl_gray8_transform_with_src_chn_mask_and_opacity(
                &(ptTask->Param.tCopyOrigMask), &this.tTransform, this.chOpacity);
        }
        else
#endif
        {
...
arm_fsm_rt_t __arm_2d_rgb565_sw_transform_with_src_mask_and_opacity(__arm_2d_sub_task_t *ptTask)
{
    ARM_2D_IMPL(arm_2d_op_trans_msk_opa_t, ptTask->ptOP);
    B_ASSERT(ARM_2D_COLOUR_RGB565 == OP_CORE.ptOp->Info.Colour.chScheme);
...
arm_fsm_rt_t __arm_2d_cccn888_sw_transform_with_src_mask_and_opacity(__arm_2d_sub_task_t *ptTask)
{
    ARM_2D_IMPL(arm_2d_op_trans_msk_opa_t, ptTask->ptOP);
    B_ASSERT(ARM_2D_COLOUR_SZ_32BIT == OP_CORE.ptOp->Info.Colour.u3ColourSZ);
    ...
    __arm_2d_sub_task_t *ptTask)
{
    ...
    __arm_2d_impl_gray8_colour_filling_mask_opacity_transform(&(ptTask->Param.tCopyOrig),
                                                              &this.tTransform, this.chOpacity);
    ...
    __arm_2d_sub_task_t *ptTask)
{
    ...
    __arm_2d_impl_rgb565_colour_filling_mask_opacity_transform(&(ptTask->Param.tCopyOrig),
                                                               &this.tTransform, this.chOpacity);
    ...
    __arm_2d_sub_task_t *ptTask)
{
    ...
    __arm_2d_impl_cccn888_colour_filling_mask_opacity_transform(&(ptTask->Param.tCopyOrig),
                                                                &this.tTransform, this.chOpacity);
...
{
    B_ASSERT(NULL != ptLocation);
    B_ASSERT(NULL != ptRegion);
...
__WEAK
def_low_lv_io(__ARM_2D_IO_TRANSFORM_GRAY8, __arm_2d_gray8_sw_transform_with_colour_keying);

__WEAK
def_low_lv_io(__ARM_2D_IO_TRANSFORM_RGB565, __arm_2d_rgb565_sw_transform_with_colour_keying);

__WEAK
def_low_lv_io(__ARM_2D_IO_TRANSFORM_CCCN888, __arm_2d_cccn888_sw_transform_with_colour_keying);

__WEAK
def_low_lv_io(__ARM_2D_IO_TRANSFORM_ONLY_GRAY8, __arm_2d_gray8_sw_transform_only);

__WEAK
def_low_lv_io(__ARM_2D_IO_TRANSFORM_ONLY_RGB565, __arm_2d_rgb565_sw_transform_only);

__WEAK
def_low_lv_io(__ARM_2D_IO_TRANSFORM_ONLY_CCCN888, __arm_2d_cccn888_sw_transform_only);
...
__WEAK
def_low_lv_io(__ARM_2D_IO_TRANSFORM_WITH_SRC_MSK_GRAY8, __arm_2d_gray8_sw_transform_with_src_mask);
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike =
                        ref_low_lv_io(__ARM_2D_IO_TRANSFORM_WITH_COLOUR_KEYING_AND_OPACITY_GRAY8),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike =
                        ref_low_lv_io(__ARM_2D_IO_TRANSFORM_WITH_COLOUR_KEYING_AND_OPACITY_RGB565),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike =
                        ref_low_lv_io(__ARM_2D_IO_TRANSFORM_WITH_COLOUR_KEYING_AND_OPACITY_CCCN888),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike =
                        ref_low_lv_io(__ARM_2D_IO_TRANSFORM_ONLY_WITH_OPACITY_CCCN888),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike =
                        ref_low_lv_io(__ARM_2D_IO_TRANSFORM_WITH_SRC_MSK_AND_OPACITY_GRAY8),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike =
                        ref_low_lv_io(__ARM_2D_IO_TRANSFORM_WITH_SRC_MSK_AND_OPACITY_RGB565),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike =
                        ref_low_lv_io(__ARM_2D_IO_TRANSFORM_WITH_SRC_MSK_AND_OPACITY_CCCN888),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike =
                        ref_low_lv_io(__ARM_2D_IO_FILL_COLOUR_WITH_MSK_OPACITY_AND_TRANSFORM_GRAY8),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike = ref_low_lv_io(
                        __ARM_2D_IO_FILL_COLOUR_WITH_MSK_OPACITY_AND_TRANSFORM_RGB565),
                    .ptFillOrigLike = NULL,
                },
        },
};
    ...
    .Info =
        {
            ...
            .LowLevelIO =
                {
                    .ptCopyOrigLike = ref_low_lv_io(
                        __ARM_2D_IO_FILL_COLOUR_WITH_MSK_OPACITY_AND_TRANSFORM_CCCN888),
                    .ptFillOrigLike = NULL,
                },
        },
};
