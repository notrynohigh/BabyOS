...
static uint_fast16_t __arm_2d_sub_task_available_count(void)
{
    ...
    arm_irq_safe
    {
        ...
        hwResult = (hwResult > ARM_2D_CTRL.hwBookCount) ? hwResult - ARM_2D_CTRL.hwBookCount : 0;
    }

    return hwResult;
}

static __arm_2d_sub_task_t *__arm_2d_sub_task_new(void)
{
...
static void __arm_2d_sub_task_free(__arm_2d_sub_task_t *ptTask)
{
    if (NULL == ptTask)
    {
        return;
...
static void __arm_2d_sub_task_add(__arm_2d_sub_task_t *ptTask)
{
    B_ASSERT(NULL != ptTask);
    // bool bIsEmpty = false;
    arm_irq_safe
    {
        // bIsEmpty = (0 == ARM_2D_CTRL.hwTaskCount);

        ARM_LIST_QUEUE_ENQUEUE(ARM_2D_CTRL.TaskFIFO.ptHead, ARM_2D_CTRL.TaskFIFO.ptTail, ptTask);
...
static __arm_2d_sub_task_t *__arm_2d_sub_task_fetch(void)
{
    __arm_2d_sub_task_t *ptTask = NULL;
    ...
    arm_irq_safe
    {
        ...
        {
            ARM_LIST_QUEUE_DEQUEUE(ARM_2D_CTRL.TaskFIFO.ptHead, ARM_2D_CTRL.TaskFIFO.ptTail,
                                   ptTask);
            ARM_2D_CTRL.hwTaskCount--;
        }
    }
...
static arm_fsm_rt_t __arm_2d_call_default_io(__arm_2d_sub_task_t *ptTask,
                                             uint_fast8_t         chInterfaceIndex)
{
    ...
    do
    {
        if (chAccPreference != ARM_2D_PREF_ACC_SW_ONLY)
        {
            ...
            B_ASSERT(tResult != arm_fsm_rt_on_going);
            ...
            if ((chAccPreference == ARM_2D_PREF_ACC_HW_ONLY) && (ARM_2D_ERR_NOT_SUPPORT == tResult))
            {
...
{
    B_ASSERT(tResult != arm_fsm_rt_async);
    B_ASSERT(tResult != arm_fsm_rt_on_going);
    B_ASSERT(tResult != arm_fsm_rt_wait_for_obj);
    ...
    if (0 == ptOP->Status.u4SubTaskCount)
    {
        ...
        {
            (ptOP->evt2DOpCpl.fnHandler)(ptOP, ptOP->tResult, ptOP->evt2DOpCpl.pTarget);
 ...
 */
void __arm_2d_notify_sub_task_cpl(__arm_2d_sub_task_t *ptTask, arm_fsm_rt_t tResult, bool bFromHW)
{
    ...
    B_ASSERT(NULL != ptTask);
    B_ASSERT(NULL != ptOP);
    B_ASSERT(ptOP->Status.u4SubTaskCount > 0);
...
#define __ARM_2D_BACKEND_TASK_RESET_FSM() \
    do                                    \
    {                                     \
        this.chState = START;             \
    } while (0);
 ...
 */
static arm_fsm_rt_t __arm_2d_backend_task(arm_2d_task_t *ptThis)
{
    ...
    switch (this.chState)
    {
        ...
        case DISPATCH:
            ...
            //! dispatch sub tasks
            this.tResult = __arm_2d_sub_task_dispatch((__arm_2d_sub_task_t *)this.ptTask);

            if ((arm_fsm_rt_on_going == this.tResult)  //!< sub task wants to yield
                || (arm_fsm_rt_wait_for_obj == this.tResult))
            {  //!< sub task wants to sync-up with applications
                return this.tResult;
            }
            ...
            if (this.tResult != arm_fsm_rt_async)
            {
                ...
                __arm_2d_notify_sub_task_cpl((__arm_2d_sub_task_t *)this.ptTask, this.tResult,
                                             false);
            }
 ...
 *! \retval (<0) Serious error is detected.
 */
static arm_fsm_rt_t __arm_2d_frontend_task(arm_2d_task_t *ptThis)
{
    ...
    arm_irq_safe
    {
        ARM_LIST_QUEUE_PEEK(ARM_2D_CTRL.OPFIFO.ptHead, ARM_2D_CTRL.OPFIFO.ptTail, ptOP);
    }
    ...
    if ((arm_fsm_rt_cpl == tResult) || (tResult < 0))
    {
        ...
        arm_irq_safe
        {
            ARM_LIST_QUEUE_DEQUEUE(ARM_2D_CTRL.OPFIFO.ptHead, ARM_2D_CTRL.OPFIFO.ptTail, ptOP);
        }
    ...
    if (arm_fsm_rt_wait_for_obj == tResult || arm_fsm_rt_wait_for_res == tResult)
    {
        return tResult;
    }

    B_ASSERT(arm_fsm_rt_on_going != tResult);
...
arm_fsm_rt_t __arm_2d_op_frontend(arm_2d_op_core_t *ptThis)
{
    ...
    arm_irq_safe
    {
        ...
        ARM_LIST_QUEUE_ENQUEUE(ARM_2D_CTRL.OPFIFO.ptHead, ARM_2D_CTRL.OPFIFO.ptTail, ptThis);
    }
 ...
 */
arm_2d_err_t __arm_2d_async_init(__arm_2d_sub_task_t *ptSubTasks, uint_fast16_t hwCount)
{
