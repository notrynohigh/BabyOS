...
extern IMPL_PFB_ON_DRAW(__pfb_draw_scene_mode_user);

static IMPL_PFB_ON_DRAW(__pfb_draw_scene_mode_fade);

static IMPL_PFB_ON_DRAW(__pfb_draw_scene_mode_slide);

static IMPL_PFB_ON_DRAW(__pfb_draw_scene_mode_erase);
...
static void __scene_player_depose_all_scene(arm_2d_scene_player_t *ptThis)
{
    ...
    do
    {
        arm_irq_safe
        {
            ARM_LIST_QUEUE_DEQUEUE(this.SceneFIFO.ptHead, this.SceneFIFO.ptTail, ptScene);
        }
        if (NULL == ptScene)
        {
            break;
...
ARM_NONNULL(1)
void arm_2d_scene_player_append_scenes(arm_2d_scene_player_t *ptThis, arm_2d_scene_t *ptScenes,
                                       int_fast16_t hwCount)
{
    B_ASSERT(NULL != ptThis);
    B_ASSERT(NULL != ptScenes);
    B_ASSERT(hwCount > 0);
    ...
    do
    {
        ...
        arm_irq_safe
        {
            ARM_LIST_QUEUE_ENQUEUE(this.SceneFIFO.ptHead, this.SceneFIFO.ptTail, ptScene);
        }
    } while (--hwCount);
}
                                              ...
                                              uint16_t                    hwSettings)
{
    ...
    /* valid input */
    B_ASSERT(((hwSettings & __ARM_2D_SCENE_SWTICH_CFG_DEFAULT_BG_msk) >>
              __ARM_2D_SCENE_SWTICH_CFG_DEFAULT_BG_pos) < 3);
...
arm_2d_err_t __arm_2d_scene_player_register_on_draw_navigation_event_handler(
    arm_2d_scene_player_t *ptThis, arm_2d_helper_draw_handler_t *fnHandler, void *pTarget,
    arm_2d_region_list_item_t *ptDirtyRegions)
{
    ...
    return arm_2d_helper_pfb_update_dependency(&ptThis->use_as__arm_2d_helper_pfb_t,
                                               ARM_2D_PFB_DEPEND_ON_NAVIGATION, &tDependency);
}
...
arm_2d_err_t __arm_2d_scene_player_register_before_switching_event_handler(
    arm_2d_scene_player_t *ptThis, arm_2d_scene_before_scene_switching_handler_t *fnHandler,
    void *pTarget)
{
    B_ASSERT(NULL != ptThis);
    B_ASSERT(NULL != fnHandler);
...
static IMPL_PFB_ON_DRAW(__pfb_draw_scene_mode_default_background)
{
    arm_2d_scene_player_t *ptThis = (arm_2d_scene_player_t *)pTarget;
...
IMPL_PFB_ON_DRAW(__pfb_draw_scene_mode_fade)
{
    ...
    __arm_2d_color_t tColour =
        (ARM_2D_SCENE_SWITCH_CFG_FADE_BLACK == this.Switch.tConfig.Feature.chMode)
    ...
    if (bIsNewFrame)
    {
        ...
        switch (this.Switch.chState)
        {
            ...
            case FADE_IN:
                ...
                hwOpacity =
                    (900ul * (int64_t)nElapsed /
                     arm_2d_helper_convert_ms_to_ticks((this.Switch.hwPeriod - hwKeepPeriod) >> 1));

                hwOpacity =
                    (uint_fast16_t)(256.0f * arm_sin_f32(ARM_2D_ANGLE((float)hwOpacity / 10.0f)));
#else
    ...
    do
    {
        ...
        {
            arm_2d_fill_colour(ptTile, NULL, tColour.tValue);
            break;
        }

        B_ASSERT(NULL != this.SceneFIFO.ptHead);
        ...
        arm_2d_fill_colour_with_opacity(ptTile, NULL, tColour, this.Switch.Fade.chOpacity);
    } while (0);
...
static void __draw_erase_scene(arm_2d_scene_player_t *ptThis, arm_2d_scene_t *ptScene,
                               const arm_2d_tile_t *ptTile, bool bIsNewFrame, bool bIgnoreBG,
                               bool bIgnoreScene)
{
    ...
    if (bIsNewFrame)
    {
        ...
        switch (this.Switch.chState)
        {
            ...
            case ERASE:
                ...
                this.Switch.Erase.iOffset =
                    ((int64_t)iTargetDistance * (int64_t)nElapsed /
                     arm_2d_helper_convert_ms_to_ticks((this.Switch.hwPeriod) >> 1));
    ...
    do
    {
        ...
        switch (this.Switch.tConfig.Feature.chMode)
        {
            case ARM_2D_SCENE_SWITCH_CFG_ERASE_LEFT:
                ...
                arm_2d_tile_generate_child(ptTile, &tWindow, &this.Switch.Erase.tTemp, false);

                tWindow.tSize.iWidth = iTargetDistance;
                arm_2d_tile_generate_child(&this.Switch.Erase.tTemp, &tWindow,
                                           &this.Switch.Erase.tSceneWindow, false);
                break;
            case ARM_2D_SCENE_SWITCH_CFG_ERASE_RIGHT:
                tWindow.tSize.iWidth -= iOffset;
        ...
        __draw_erase_scene(ptThis, ptScene, &this.Switch.Erase.tSceneWindow, bIsNewFrame,
    ...
    do
    {
        ...
        switch (this.Switch.tConfig.Feature.chMode)
        {
            case ARM_2D_SCENE_SWITCH_CFG_SLIDE_LEFT:
                ...
                arm_2d_tile_generate_child(ptTile, &tWindow, &this.Switch.Slide.tSceneWindow,
                                           false);
                break;
            case ARM_2D_SCENE_SWITCH_CFG_SLIDE_RIGHT:
                tWindow.tLocation.iX = this.Switch.Slide.iOffset;
        ...
        __draw_erase_scene(ptThis, ptScene, &this.Switch.Slide.tSceneWindow, bIsNewFrame,
                           this.Switch.tConfig.Feature.bIgnoreOldSceneBG,
                           this.Switch.tConfig.Feature.bIgnoreOldScene);
...
arm_fsm_rt_t arm_2d_scene_player_task(arm_2d_scene_player_t *ptThis)
{
    ...
    switch (this.Runtime.chState)
    {
            ...
            if (NULL == ptScene)
            {
                ...
                if (bNextSceneReq)
                {
                    ...
                    ARM_2D_INVOKE_RT_VOID(
                        ...
                        ARM_2D_PARAM(this.Events.evtBeforeSwitching.pTarget, ptThis, ptScene));
                    /* try again */
                    break;
                }
        ...
        case DRAW_BACKGROUND_PREPARE:
            if (NULL == ptScene->fnBackground)
            {
                ...
                if (NULL != ptScene->ptDirtyRegion && NULL != ptScene->fnScene)
                {
                    ...
                    ARM_2D_HELPER_PFB_UPDATE_ON_DRAW_HANDLER(&this.use_as__arm_2d_helper_pfb_t,
                                                             ptScene->fnScene, ptScene);
            ...
            else
            {
                ...
                ARM_2D_HELPER_PFB_UPDATE_ON_DRAW_HANDLER(&this.use_as__arm_2d_helper_pfb_t,
                                                         ptScene->fnBackground, ptScene);
                this.Runtime.chState = DRAW_BACKGROUND;
            }
            // fall-through

        case DRAW_BACKGROUND:
            tResult = arm_2d_helper_pfb_task(&this.use_as__arm_2d_helper_pfb_t, NULL);
            if (tResult < 0)
            {
        ...
        case DRAW_SCENE:
            tResult =
                arm_2d_helper_pfb_task(&this.use_as__arm_2d_helper_pfb_t, ptScene->ptDirtyRegion);
            if (tResult < 0)
            {
                ARM_2D_USER_SCENE_PLAYER_TASK_RESET();
                return tResult;
        ...
        case POST_SCENE_CHECK:
            ...
            if (this.Runtime.bNextSceneReq)
            {
                ...
                if ((ARM_2D_SCENE_SWITCH_CFG_NONE == this.Switch.tConfig.Feature.chMode) ||
                    (this.Switch.tConfig.Feature.chMode >= __ARM_2D_SCENE_SWITCH_CFG_VALID) ||
        ...
        case SWITCH_SCENE_PREPARE:
        {
            ...
            B_ASSERT(this.Switch.ptMode->chEffects != ARM_2D_SCENE_SWITCH_CFG_NONE);

            /* update drawer */
            ARM_2D_HELPER_PFB_UPDATE_ON_DRAW_HANDLER(&this.use_as__arm_2d_helper_pfb_t,
                                                     this.Switch.ptMode->fnSwitchDrawer, ptThis);
            ...
            this.Switch.hwPeriod =
                MAX(__ARM_2D_CFG_HELPER_SWITCH_MIN_PERIOD__, this.Switch.hwPeriod);
