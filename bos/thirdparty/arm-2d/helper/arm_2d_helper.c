...
extern uintptr_t arm_2d_port_new_semaphore(void);

extern void arm_2d_helper_rtos_init(void);
...
ARM_NONNULL(4, 5)
bool __arm_2d_helper_time_liner_slider(int32_t nFrom, int32_t nTo, int64_t lPeriod,
                                       int32_t *pnStroke, int64_t *plTimestamp)
{
    B_ASSERT(NULL != plTimestamp);
    B_ASSERT(NULL != pnStroke);
...
ARM_NONNULL(5, 6)
bool __arm_2d_helper_time_cos_slider(int32_t nFrom, int32_t nTo, int64_t lPeriod, float fPhase,
                                     int32_t *pnStroke, int64_t *plTimestamp)
{
    int64_t lTimestamp = arm_2d_helper_get_system_timestamp();
    B_ASSERT(NULL != plTimestamp);
...
uint32_t __arm_2d_helper_colour_slider(uint32_t wFrom, uint32_t wTo, int32_t nDistance,
                                       int32_t nOffset)
{
    B_ASSERT(nDistance > 0);
 ...
 */
ARM_NONNULL(4, 5)
bool __arm_2d_helper_time_half_cos_slider(int32_t nFrom, int32_t nTo, int64_t lPeriod,
                                          int32_t *pnStroke, int64_t *plTimestamp)
{
...
ARM_NONNULL(1, 3)
bool arm_2d_helper_pi_slider(arm_2d_helper_pi_slider_t *ptThis, int32_t nTargetPosition,
                             int32_t *pnResult)
{
    int64_t lTimestamp = arm_2d_helper_get_system_timestamp();
    B_ASSERT(NULL != ptThis);
    B_ASSERT(NULL != pnResult);
    bool bResult = false;
...
void arm_2d_helper_draw_box(const arm_2d_tile_t *ptTarget, const arm_2d_region_t *ptRegion,
                            int16_t iBorderWidth, COLOUR_INT tColour, uint8_t chOpacity)
{
    ...
    arm_2d_fill_colour_with_opacity(ptTarget, &tDrawRegion, (__arm_2d_color_t){tColour}, chOpacity);
...
void arm_2d_helper_film_set_frame(arm_2d_helper_film_t *ptThis, int32_t nIndex)
{
    ...
    ptFrame->tRegion.tLocation.iX = (nIndex % this.hwColumn) * ptFrame->tRegion.tSize.iWidth;
    ptFrame->tRegion.tLocation.iY = (nIndex / this.hwColumn) * ptFrame->tRegion.tSize.iHeight;
}
