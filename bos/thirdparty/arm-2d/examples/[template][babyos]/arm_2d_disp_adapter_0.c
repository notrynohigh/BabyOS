...
/*============================ PROTOTYPES ====================================*/
extern int32_t Disp0_DrawBitmap(int16_t x, int16_t y, int16_t width, int16_t height,
...
static IMPL_PFB_ON_DRAW(__pfb_draw_handler)
{
    ...
    arm_2d_canvas(ptTile, __top_container)
    {
        ...
        {
            draw_round_corner_box(ptTile, &__centre_region, GLCD_COLOR_BLACK, 64, bIsNewFrame);
        }
...
static IMPL_PFB_ON_DRAW(__pfb_draw_navigation)
{
    ARM_2D_UNUSED(pTarget);
    ...
    if (__DISP0_CFG_ITERATION_CNT__)
    {
        arm_2dp_fill_colour_with_opacity(
            ...
            (arm_2d_region_t[]){
                {
                    .tLocation = {.iX = 0, .iY = __DISP0_CFG_SCEEN_HEIGHT__ - 17},
                    .tSize =
                        {
            ...
            (__arm_2d_color_t){__RGB(64, 64, 64)}, 255 - 32);
        arm_2d_op_wait_async(NULL);
    ...
    arm_lcd_printf("arm-2d v" ARM_TO_STRING(ARM_2D_VERSION_MAJOR) "." ARM_TO_STRING(
        ARM_2D_VERSION_MINOR) "." ARM_TO_STRING(ARM_2D_VERSION_PATCH) "-" ARM_2D_VERSION_STR);
#endif
...
{
    arm_2d_helper_pfb_report_rendering_complete(&DISP0_ADAPTER.use_as__arm_2d_helper_pfb_t);
}
...
IMPL_PFB_ON_LOW_LV_RENDERING(__glcd0_pfb_render_handler)
{
    ...
    __disp_adapter0_request_async_flushing(
        pTarget, bIsNewFrame, ptTile->tRegion.tLocation.iX, ptTile->tRegion.tLocation.iY,
        ptTile->tRegion.tSize.iWidth, ptTile->tRegion.tSize.iHeight,
...
IMPL_PFB_ON_LOW_LV_RENDERING(__glcd0_pfb_render_handler)
{
    ...
    Disp0_DrawBitmap(ptTile->tRegion.tLocation.iX, ptTile->tRegion.tLocation.iY,
                     ptTile->tRegion.tSize.iWidth, ptTile->tRegion.tSize.iHeight,
...
static bool __on_each_frame_complete(void *ptTarget)
{
    ...
    int32_t nTotalLCDCycCount =
        DISP0_ADAPTER.use_as__arm_2d_helper_pfb_t.Statistics.nRenderingCycle;
...
static void __user_scene_player_init(void)
{
...
#if __DISP0_CFG_VIRTUAL_RESOURCE_HELPER__ && !__DISP0_CFG_USE_HEAP_FOR_VIRTUAL_RESOURCE_HELPER__
                + 3
#endif
            ,
            {
...
#if __DISP0_CFG_VIRTUAL_RESOURCE_HELPER__ && !__DISP0_CFG_USE_HEAP_FOR_VIRTUAL_RESOURCE_HELPER__
            .FrameBuffer.u4PoolReserve =
                3,  // reserve 3 PFB blocks for the virtual resource service
#endif
            ) < 0)
    {
        //! error detected
    ...
    arm_lcd_text_init((arm_2d_region_t[]){{.tSize = {
                                               .iWidth  = __DISP0_CFG_SCEEN_WIDTH__,
...
void disp_adapter0_init(void)
{
    ...
    do
    {
        ...
        ADD_LAST_REGION_TO_LIST(s_tNavDirtyRegionList,
            ...
            arm_2d_scene_player_register_on_draw_navigation_event_handler(
                &DISP0_ADAPTER, __pfb_draw_navigation, NULL,
    ...
    if (!__DISP0_CFG_DISABLE_DEFAULT_SCENE__)
    {
        ...
        arm_2d_scene_player_append_scenes(&DISP0_ADAPTER, (arm_2d_scene_t *)s_tScenes,
                                          dimof(s_tScenes));
    }
}
 ...
 *----------------------------------------------------------------------------*/
void __disp_adapter0_vres_read_memory(intptr_t pObj, void *pBuffer, uintptr_t pAddress,
                                      size_t nSizeInByte)
{
    ARM_2D_UNUSED(pObj);
...
intptr_t __disp_adapter0_vres_asset_loader(uintptr_t pObj, arm_2d_vres_t *ptVRES,
                                           arm_2d_region_t *ptRegion)
{
    ...
    if (nBitsPerPixel < 8)
    {
        ...
        {
            __disp_adapter0_vres_read_memory(pObj, (void *)pDes, (uintptr_t)pSrc, nBytesPerLine);
    ...
    else
    {
        ...
        for (int_fast16_t y = 0; y < ptRegion->tSize.iHeight; y++)
        {
            __disp_adapter0_vres_read_memory(pObj, (void *)pDes, (uintptr_t)pSrc,
...
void __disp_adapter0_vres_buffer_deposer(uintptr_t pTarget, arm_2d_vres_t *ptVRES, intptr_t pBuffer)
{
#if __DISP0_CFG_USE_HEAP_FOR_VIRTUAL_RESOURCE_HELPER__
    ARM_2D_UNUSED(pTarget);
    ARM_2D_UNUSED(ptVRES);
