...
#define DMA_CSELR_BASE (*((volatile uint32_t *)(DMA1_BASE_ADDR + 0x000000A8UL)))
...
int bMcuDmaConfig(bHalDmaConfig_t *pconf)
{
    ...
    if (pconf->request == B_DMA_REQ_UART1_RX &&
        (pconf->chl == B_HAL_DMA_CHL_3 || pconf->chl == B_HAL_DMA_CHL_5))
    {
        B_MODIFY_REG(DMA_CSELR_BASE, 0xf << pconf->chl, 0x3 << pconf->chl);
    }
    else if (pconf->request == B_DMA_REQ_UART2_RX &&
        (pconf->chl == B_HAL_DMA_CHL_5 || pconf->chl == B_HAL_DMA_CHL_6))
    {
        B_MODIFY_REG(DMA_CSELR_BASE, 0xf << pconf->chl, 0x4 << pconf->chl);
    }
    else if (pconf->request == B_DMA_REQ_LPUART1_RX &&
        (pconf->chl == B_HAL_DMA_CHL_3 || pconf->chl == B_HAL_DMA_CHL_6))
    {
        B_MODIFY_REG(DMA_CSELR_BASE, 0xf << pconf->chl, 0x5 << pconf->chl);
    }
    else
    {
        return -1;
